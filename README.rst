...
To do this, we need to set the ``USE\_LD`` flag, for example by (replace with your favorite compiler):
...
When ``USE\_LD`` is turned on, all "tricks" will essentially evaluate to NOP, and all calls made with the reserved ``dnload\_`` prefix will simply call the functions as normal.
...
If the macro ``USE\_LD`` would be defined, all this would simply evaluate to a self-explanatory hello world program:
