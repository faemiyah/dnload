...
"}"
"vec3 forward=normalize(uniform_array[1]),right=normalize(cross(forward,uniform_array[2])),direction=normalize(aspect.x*right+aspect.y*normalize(cross(right,forward))+forward);"
"float product=dot(-uniform_array[0],direction),radius=1.+sin(uniform_array[3].x/4444.)*.1;"
