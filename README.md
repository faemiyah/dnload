####dnload
<a href="https://raw.githubusercontent.com/trilkk/dnload/master/screenshot_www.png"><img src="https://raw.githubusercontent.com/trilkk/dnload/master/screenshot_www.png" height="130em" align="right" /></a>
...
The script is used for two purposes:
  ...
  * Generating a header file to hide the complexities of size-limited linking. This can be also be on systems where compilation is not supported. The main use of this feature is to allow demo developers to work on other platforms except the main targets. The generated header tries to preserve portability.
...
To do this, we need to set the `USE``_``LD` flag, for example by (replace with your favorite compiler):
...
When `USE_LD` is turned on, all 'tricks' will essentially evaluate to NOP, and all calls made with the reserved `dnload``_` prefix will simply call the functions as normal.
...
If the macro `USE``_``LD` would be defined, all this would simply evaluate to a self-explanatory hello world program:
...
This section of the documentation explores both the current and historical methods of reducing executable file size on `*`nix systems. If you are only interested in the current 'best practice' operation of the script, you can [#Current_compression_procedure skip directly therein].
...
Scouring old releases, the first instance of a `*`nix 4k using a self-dumping shell script seems to be _helsinki-spiegelberg by tsygä_ [#References [12]]. However, instead of dumping an executable binary, this entry actually unpacks to a source and compiles before execution.

There are some other variants afterwards, but the first use of the modern one-line filedump is found in _Yellow Rose of Texas by Fit & Bandwagon_ [#References [13]]. Consequently, this is also the first 'big' Linux 4k intro.
...
There are multiple programs that crunch classic gzip files to as small sizes as possible, but modern `*`nix systems come with better compressors and better tricks. For example, using a trick from _ts/TDA_ [#References [27]], redefining HOME allows using the tilde character for clever reduction:
...
One could assume that `xz` format would be more advantageous than the earlier `lzma`, but this seems not to be the case as `lzma` compression headers are smaller [#References [28]][#References [29]].
...
This is relevant, because the symbol tables consume quite a lot of space. Taking a look into the FreeBSD system headers [#References [17]] we can examine the symbol structs (going into `.dynsym`):
...
Luckily, using the forementioned two symbols, we can perform dynamic loading ourselves. _Yellow Rose_ loaded only the GL functions this way, but using a clever arrangement of text, we can embed the library information in a text block at practically no cost:
...
Fortunately, there is a better way. In 2008, Pouet users *parcelshit* [#References [3]] and *las/Mercury* [#References [4]] published code to search for symbols in ELF32 shared objects by their hashed names. Some of the code linked in the discussion [#References [19]] seems to be unaccessible by now, but at least the original import-by-hash implementation [#References [18]] (used as proof-of-concept for our implementation) seems to still be available.
...
The authors have been kind enough to _literally_ point out where the hacking should begin. `rtld.c` line 334:
...
We obviously do not want to replace our own rtld with a crappy, hacked version so it's better to use the custom version directly. Reading _A Whirlwind Tutorial on Creating Really Teensy ELF Executables for Linux_ [#References [2]] and _Executable and Linkable Format (ELF)_ [#References [16]], we can construct the minimal program to do this and start the investigation:
...
The `maximum` compression mode uses the following techniques to decrease binary size:
...
Traditionally programs put uninitialized but statically reserved memory in a section named `.bss` or _Block Started by Symbol_ that will be handled by the linker. We naturally do not have anything of the like. However, the program header PT_LOAD provides a possibility to set the size on disk (`p_filesz`) as different from size in memory (`p_memsz`).
...
*Note:* This will fail with a _bus error_ if the size of fake `.bss` segment nears 128 megabytes. If this happens, the solution is to do the exact same thing `ld` would do - construct another `PT_LOAD` segment at the next memory page without the execute flag set and assign all uninitialized memory there instead.

=== Entry into and exit from `_start` ===

As described [#Example_program earlier], we take care from entry and exit from the `_start` procedure ourselves. The compiler will happily push registers at the beginning and subtract the stack at the end of the function. On i386, this looks like the following (from `hello_world.cpp` example):
...
As Brian Raiter already noted in _A Whirlwind Tutorial on Creating Really Teensy ELF Executables for Linux_ [#References [2]], it does not really seem to matter if the ELF headers overlap. The structures will be assigned addresses exactly based on the offsets given, and they can freely overlap with other structs in memory.
...
*Note:* Interleaving with `DT_DEBUG` is dangerous, as the structure will be filled on runtime as seen in [#Import_by_hash_-_scouring_ELF headers an earlier section]. In practice, it seems to not cause problems currently.

=== Entry point ===

Default ELF entry point is fixed to address `0x8048000`. This can be changed to a better compressable address.

The only issue here is that specifying `--entry` to `ld` does not actually change the entry point (it probably would if `ld` would also construct the headers). We need to modify linker scripts. This is done the same way as [Import_by_hash_-_scouring_ELF32_headers earlier] - make the linker export the full linker script and change the `SEGMENT_START` directives into a better constant (`0x2000000` at the time of writing).
...
The obvious solution is to only have one bucket and point it at the end of the chain array, then have the chain array count down from this index, going through the symbols one by one. It's definitely ineffective, but that hardly matters. This results as a generation algorithm as follows:
...
In his similar project [#References [32]], _minas/Calodox_ uses the manually constructed headers to directly know the location into which the dynamic linker will write this address.
...
Luckily, _minas/calodox_ noticed that `DT_STRTAB` and `DT_SYMTAB` seem to have two interesting relations:
...
However, as Amand Tihon [#References [23]] points in his own similar project [#References [24]], on Linux the whole of symbol table can be omitted. This is done by having the `DT_SYMTAB` dynamic structure entry point to address value `0` and omitting `DT_HASH` completely. All in all, this means that size-optimized binaries on Linux are [#Minimal_DT_HASH 99 bytes] smaller than on FreeBSD. Interleaving of headers takes away some of this advantage, in practice it seems to be about 30 compressed bytes.
...
This script would not have been possible without the prior work done by various other parties. Especially the following entities deserve kudos for their efforts:
  ...
  * *Amand Tihon* [#References [23]] for _BOLD - The Byte Optimized Linker_ [#References [24]] and noticing that `DT_SYMTAB` can be empty.
  * *ts/TDA* [#References [26]] for `INT 3` exit, `HOME=` -shell script trick, and probably something else.
  * *minas/calodox* [#References [31]] for _elfling_ [#References [32]] and various symbol scouring tricks.
...
The list might be missing some parties, who are thus not getting the credit they deserve. If you notice this, please help correct the errors.
...
The name `USE``_``LD` is legacy, which has preserved unchanged from earlier Faemiyah prods. You may change the definition with the `-d` or `--definition` command line argument when invoking the script.

==== Do I need to use `_start` ? ====

When manually creating the program headers, the symbol would not necessarily need to be named `_``start` - it could be anything, and the name will be stripped out anyway. However, this is a known convention.
...
= TODO =
  * Should probably create the header file in a smart(er) manner if it is not found.
  * Add cross-compilation support, at the very least between `*`nix systems at the "maximum" operation mode.
...
<a name="reference_1"></a>`[`1`]` [http://www.pouet.net/user.php?who=4078 Marq/Fit in Pouet]<br>
...
`[`15`]` [http://pubs.opengroup.org/onlinepubs/009695399/functions/dlsym.html dlsym specification at The Open Group Base Specifications]<br>
`[`16`]` [http://www.skyfree.org/linux/references/ELF_Format.pdf Executable and Linkable Format (ELF) specification]<br>
`[`17`]` [http://svnweb.freebsd.org/base/stable/10/sys/sys/ FreeBSD system headers]<br>
`[`18`]` [http://pastebin.com/f479f8beb parcelshit's original import-by-hash implementation]<br>
`[`19`]` [http://www.pouet.net/topic.php?which=5392&page=4 Haxxoring the ELF format for 1k/4k stuff -thread on Pouet]<br>
`[`20`]` [http://programmers.stackexchange.com/questions/49550/which-hashing-algorithm-is-best-for-uniqueness-and-speed Which hashing algorithm is best for uniqueness and speed? - Ian Boyd's answer at Programmers Stack Exchange]<br>
`[`21`]` [http://www.pouet.net/prod.php?which=61753 Inconsistency by Hymy in Pouet]<br>
`[`22`]` [http://www.pouet.net/prod.php?which=59264 tutorial2 by Ye Olde Laptops Posse in Pouet]<br>
`[`23`]` [http://www.alrj.org/ Amand Tihon]<br>
`[`24`]` [http://www.alrj.org/projects/bold/ BOLD - The Byte Optimized Linker]<br>
`[`25`]` [https://blogs.oracle.com/ali/entry/gnu_hash_elf_sections GNU Hash ELF Sections by Ali Bahrami]<br>
`[`26`]` [http://www.pouet.net/user.php?who=58551 ts/TDA in Pouet]<br>
`[`27`]` [http://www.pouet.net/topic.php?which=10038&page=2#c482906 File-dumping example by ts/TDA]<br>
`[`28`]` [http://svn.python.org/projects/external/xz-5.0.3/doc/lzma-file-format.txt LZMA file format]<br>
`[`29`]` [http://tukaani.org/xz/xz-file-format-1.0.4.txt XZ file format]<br>
`[`30`]` [http://x86.renejeschke.de/html/file_module_x86_id_142.html INT 3 instruction at René Jeschke's x86 instruction set reference mirror]<br>
`[`31`]` [http://www.pouet.net/user.php?who=6102 minas/calodox in Pouet]<br>
`[`32`]` [https://github.com/google/elfling elfling]<br>